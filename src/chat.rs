...
fn parse_envelope(raw: &str) -> Option<serde_json::Value> {
    if let Ok(env) = serde_json::from_str::<serde_json::Value>(raw)
        && env["v"].is_string() && env["text"].is_string() {
            return Some(env);
        }
...
pub fn role_claim(
    ...
    if let Some(existing) = store::get_role_lease(&room.room_id, role)
        && existing.agent_id != me && existing.lease_expires > now_ts {
            return Err(format!(
...
pub fn role_heartbeat(
    ...
    if let Some(current) = &existing
        && current.agent_id != me && current.lease_expires > now_ts {
            return Err(format!(
                "Role '{}' is currently held by '{}' for another {}s.",
                role,
                current.agent_id,
    ...
    if !tasks.iter().any(|t| t.id.starts_with(task_id)) {
        ...
        for msg in &msgs {
            if msg["type"].as_str() == Some("bounty")
                && msg["id"]
                    .as_str()
                    .is_some_and(|id| id.starts_with(task_id))
            {
                tasks.push(store::Task {
...
mod tests {
    use super::{
        ...
        discovery_decay_weight, encrypt_envelope, enforce_outbound_plaza_rate_limit,
        infer_soma_subject_path, ingest_auxiliary_event, list_role_leases, list_work_receipts,
        make_envelope, make_invite_redemption, payment_complete_solana_deposit, pin, pins,
        resolve_room, role_claim, role_heartbeat, role_release, seed_plaza_rate_limit_state,
        send_watch_heartbeat, should_display_message, signing_message_bytes, soma_churn_decay,
        soma_correct, stale_claim_weight, task_add, task_add_with_oracle, task_checkpoint,
        task_done, unpin, verified_solana_deposit_from_tx,
    };
    use crate::crypto;
    ...
    fn bounty_verify_awards_credits_to_winner() {
        ...
        let task_id =
            task_add_with_oracle("Build feature X", Some("true"), Some(50), None, None).unwrap();
    ...
    fn bounty_submit_rejects_self_dealing() {
        ...
        let task_id =
            task_add_with_oracle("Self-deal test task", None, Some(50), None, None).unwrap();

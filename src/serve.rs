    ...
    match (method, segments.as_slice()) {
        ...
        ("POST", ["api", "sandbox", "create"]) => {
            ...
            match sandbox::create(&verified_agent_id) {
                Ok(session) => {
                    // Persist so exec/destroy can verify ownership
                    crate::store::save_session(&session);
        ...
        ("POST", ["api", "sandbox", "exec"]) => {
            ...
            // Verify this agent owns the session (if we have a record)
            if let Some(session) = crate::store::load_session(&session_id) {
                if session.agent_id != verified_agent_id {
        ...
        ("POST", ["api", "sandbox", "destroy"]) => {
            ...
            match sandbox::destroy(&session_id, &provider) {
                Ok(()) => {
                    crate::store::remove_session(&session_id);
